...
#Merging the xTest, yTest and subjectTest 
testData = cbind(yTest,subjectTest,xTest);


# Binding training and test data set
finalData = rbind(trainingData,testData);


# select the desired mean() & stddev() columns
colNames  = colnames(finalData); 

# 2. Extract only the measurements on the mean and standard deviation for each measurement. 

# logic vector incluede TRUE values for the ID, mean() & stddev() columns and FALSE for others
...
# Merge finalData acitivityType include activity names
...
# finalDataNoActivityType without the activityType 
finalDataNoActivityType  = finalData[,names(finalData) != 'activityType'];

# Summarizing the finalDataNoActivityType include just the mean of each variable for each activity and each subject
